...
function ProductList({ onHomeClick }) {
  ...
  const ProductItem = (props) => {
    const { name, image, cost, description } = props;
    const isItemOnCart = cartItems.find(item => item.name === name);
    return (
      <li className="product-card">
        <h3 className="product-title">{name}</h3>
        <img src={image} alt="plant image" className="product-image" />
        <p className="product-price">{cost}</p>
        <p>{description}</p>
        <button
          className={`product-button ${isItemOnCart ? "added-to-cart" : ""}`}
          onClick={() => handleAddItemToCart(props)}
          disabled={isItemOnCart ? true : false}
        >
            ...
            <div className="product-grid" key={index}>
              ...
              <ul className="product-list">
                {category.plants.map((plant, plantIndex) => (
                  <ProductItem key={plantIndex} {...plant} />
                ))}
              </ul>
            </div>
          );
  ...
  return (
    <div>
      <Navbar />
      {showPlants && <Products />}
      {showCart && <CartItem onContinueShopping={handlePlantsClick} />}
    </div>
  );
}
